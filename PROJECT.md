...
- **Cart Logic**: A global state module managing items added to the cart, quantity, and updating the cart badge and modal in real-time.
...
| M2 | Interactive e-Catalog | Implement slider/flipbook navigation, extensible data structure, product hotspots, and product detail modal/tooltip with "Add to cart". | M1 | PLANNED |
| M3 | Berry's Calculator | Build formulation tool with unit toggle, dynamic row add/delete, real-time recalculation, license footer, and disabled checkout integration. | M1 | PLANNED |
| M4 | Cart & Global Integration | Connect cart/search icons to active functionality (adding products, displaying cart summary, micro-animations, search filter). | M2, M3 | PLANNED |
...
| M6 | E2E Pass & Hardening | Pass 100% of E2E tests, followed by Tier 5 adversarial testing and coverage hardening. | M4, M5 | PLANNED |
...
- `cart.getItems()` -> returns array of `{ product, quantity }`
...
- `js/cart.js` — Cart state and actions
